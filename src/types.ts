...
export type UpdateFlashcardDto = Partial<
  Pick<FlashcardDto, 'front_text' | 'back_text' | 'source'>
>;
...
export type DeleteFlashcardResponseDto = { message: string };
...
export type GenerationSuggestionDto = Pick<
  Tables<'flashcards'>,
  'front_text' | 'back_text' | 'source'
>;

// Request payload for POST /generations
export type CreateGenerationRequestDto = Pick<
...
export type GenerationDto = Pick<
  Tables<'generations'>,
  'id' | 'model' | 'duration' | 'flashcards_count' | 'accepted_edited_count' | 'accepted_unedited_count' | 'created_at' | 'updated_at'
>;
...
export type GenerationErrorLogsResponseDto = {
